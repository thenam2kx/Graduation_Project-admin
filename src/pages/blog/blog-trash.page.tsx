...
export default function BlogTrashPage() {
  ...
  const { data, isLoading, refetch } = useQuery({
    queryKey: ['blogs-trash', searchText],
    queryFn: async () => {
      const params = `?current=1&pageSize=50${searchText ? `&qs=${searchText}` : ''}`
      const response = await axios.get(`/api/v1/blogs/trash${params}`)
      return response.data
    }
  })

  const restoreMutation = useMutation({
    mutationFn: (id: string) => axios.patch(`/api/v1/blogs/restore/${id}`),
    onSuccess: () => {
  ...
  const forceDeleteMutation = useMutation({
    mutationFn: (id: string) => axios.delete(`/api/v1/blogs/force-delete/${id}`),
    onSuccess: () => {
  ...
  const columns = [
    ...
    { title: 'Trạng thái', key: 'status', render: () => <Tag color="red">Đã xóa</Tag> },
    {
      title: 'Thao tác', key: 'action', render: (_: any, record: any) => (
        <Space>
          <Popconfirm title="Khôi phục bài viết này?" onConfirm={() => restoreMutation.mutate(record._id)}>
            <Button type="primary" icon={<UndoOutlined />} size="small">Khôi phục</Button>
          </Popconfirm>
          <Popconfirm title="Xóa vĩnh viễn bài viết này?" onConfirm={() => forceDeleteMutation.mutate(record._id)}>
    ...
    <Card title="Thùng rác bài viết">
      ...
      <Table
        ...
        pagination={{
          ...
          showTotal: (total) => `Tổng ${total} bài viết đã xóa`
        }}
      />
    </Card>
  )
